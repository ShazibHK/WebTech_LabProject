...
export const updateTransaction= async (req, res) => {
    ...
    {
        const updateDeal = { number: parseInt(postTransaction.sales)+ parseInt(value), _id: id };
        await PostTransaction.findByIdAndUpdate(id, updateDeal, { new: true });
        res.json(updateDeal);
    }
    else if(type==="income")
    {
        const updateDeal = { number: parseInt(postTransaction.income)+ parseInt(value)/100, _id: id };
        await PostTransaction.findByIdAndUpdate(id, updateDeal, { new: true });
        res.json(updateDeal);
    }
